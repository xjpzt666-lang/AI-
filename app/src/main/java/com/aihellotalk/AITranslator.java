...
public class ChatHook {
    ...
    // ★ 全局多图雷达弹匣：最多缓存最近渲染的 3 张图片路径，防止 OOM
    private static final LinkedList<String> recentImages = new LinkedList<>();
    private static volatile long latestRenderedImageTime = 0;

    public static void install(ClassLoader cl) {
        log("=== Hook v55.0 (多图记忆雷达版) ===");
                            ...
                            if (filePathObj instanceof String) {
                                ...
                                if (imgFile.exists() && imgFile.length() > 0) {
                                    synchronized (recentImages) {
                                        // 避免同一张图片重复加入
                                        recentImages.remove(filePath);
                                        recentImages.add(filePath);
                                        // 弹匣容量限制为 3 张，超出则挤掉最老的一张
                                        if (recentImages.size() > 3) {
                                            recentImages.removeFirst();
                                        }
                                    }
                                    latestRenderedImageTime = System.currentTimeMillis();
                                    log("【UI视觉多图雷达】锁定屏幕图片: " + filePath);
                                }
                            }
                        }
                    }
    ...
    private static void addTranslateBtn(ViewGroup layout, EditText edit) {
        ...
        btn.setOnClickListener(v -> {
            ...
            // ★ 视觉绝杀V55：支持最多 3 张最近渲染的图片同时输送给大模型！
            synchronized (recentImages) {
                if (!recentImages.isEmpty()) {
                    ...
                    if (isRecent || isAsking) {
                        for (String path : recentImages) {
                            textToTranslate += "\n[LOCAL_IMAGE:" + path + "]";
                        }
                        log("【视觉强绑定】已将 " + recentImages.size() + " 张屏幕图片拼接到本次翻译请求中");
                        
                        // 用完后，如果是正常聊天就不再继续带这些图了，防止后续流量浪费；如果是提问可以留着
                        if (!isAsking) {
                            recentImages.clear();
                        }
                    }
